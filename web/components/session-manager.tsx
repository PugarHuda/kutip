...
export function SessionManager({
  ...
  onSessionChange?: (session: SessionDto | null) => void;
}) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/session?user=${address}`, { cache: "no-store" });
      const json = (await res.json()) as { session: SessionDto | null };
  ...
  async function createSession() {
    ...
    const validUntil = BigInt(Math.floor(Date.now() / 1000) + hoursN * 3600);
    ...
    try {
      ...
      const res = await fetch("/api/session", {
        ...
        body: JSON.stringify({
          intent: {
            ...intent,
            maxPerQueryUSDC: intent.maxPerQueryUSDC.toString(),
            dailyCapUSDC: intent.dailyCapUSDC.toString(),
      ...
      if (!res.ok) throw new Error(json.error ?? "Failed to store session");
  ...
  async function revoke() {
    ...
    try {
      await fetch("/api/session", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: session.id, caller: address })
      });
      setSession(null);
    } finally {
      setBusy(false);
  ...
  const active =
    session &&
    !session.revokedAt &&
    BigInt(session.validUntil) > BigInt(Math.floor(Date.now() / 1000));

  if (active && session) {
    const cap = BigInt(session.dailyCapUSDC);
    const spent = BigInt(session.spentToday);
    const pct = cap > 0n ? Number((spent * 10000n) / cap) / 100 : 0;
    const hoursLeft = Math.max(
      0,
      Math.floor(
        (Number(session.validUntil) - Math.floor(Date.now() / 1000)) / 3600
      )
    );
      ...
      >
        ...
        <div className="flex justify-between items-baseline mb-1">
          ...
          <span className="t-mono-sm font-semibold">
            {formatUSDC(session.spentToday)} / {formatUSDC(session.dailyCapUSDC)} USDC
          </span>
        </div>
        <div
        ...
        <div className="flex justify-between items-center mt-2.5">
          <span className="t-mono-sm ink-3">
            Max per query · {formatUSDC(session.maxPerQueryUSDC)} USDC
          </span>
          <button
            type="button"
            onClick={revoke}
        ...
        <div className="t-mono-sm ink-3 mt-1.5 break-all">Session id {session.id}</div>
      </div>
    );
  }
